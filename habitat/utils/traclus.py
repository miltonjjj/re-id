...
def get_point_projection_on_line(point, line):#点投影到直线上
    """
...
def partition2segments(partition):#将分割后的轨迹点连接成线段
    """
...
# Angular Distance
def d_angular(l1, l2, directional=True):#计算两直线夹角
    """
    ...
    if 0 <= theta < (90 * np.pi / 180):
        return np.sin(theta) * d_euclidean(l_longer[0], l_longer[-1])#夹角小的时候考虑线段长度的影响，返回夹角正弦与较长线段的乘积
    elif (90 * np.pi / 180) <= theta <= np.pi:
        return np.sin(theta)#夹角大的时候只返回夹角的正弦值，忽略线段长度的影响
    else:
        raise ValueError("Theta is not in the range of 0 to 180 degrees.")

# Total Trajectory Distance权重默认相同
...
# Minimum Description Length MDL计算编码成本
...
# Trajectory Smoothing移动平均滤波器平滑轨迹
...
def partition(trajectory, directional=True, progress_bar=False, w_perpendicular=1, w_angular=1):
    ...
    while start_idx + length < traj_len:
        ...
        if cost_par > cost_nopar:#当分割成本大于无分割成本时选择分割，即低复杂度（直线）继续扩展，高复杂度（转弯）及时分割
...
def get_representative_trajectory(lines, min_lines=3):#扫描线算法生成代表性轨迹
    """
...
def traclus(trajectories, max_eps=None, min_samples=10, directional=True, use_segments=True, clustering_algorithm=OPTICS, mdl_weights=[1,1,1], d_weights=[1,1,1], progress_bar=False):
    ...
    representative_trajectories = []#形成代表性轨迹
