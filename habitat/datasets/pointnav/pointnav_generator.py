...
def get_local_occupancy_grid(
    ...
    cell_size_meters: float = 0.1,
) -> List[List[int]]:
    """
    Generates a local occupancy grid around the center point.
    1: Free space
    0: Obstacle
    """
    num_cells = int(map_size_meters / cell_size_meters)
    grid = np.zeros((num_cells, num_cells), dtype=np.uint8)
    center_y = center[1]

    for i in range(num_cells):
        for j in range(num_cells):
            dx = (i - num_cells / 2) * cell_size_meters
            dz = (j - num_cells / 2) * cell_size_meters

            query_point = np.array(
                [center[0] + dx, center_y, center[2] + dz], dtype=np.float32
            )

            snapped = sim.pathfinder.snap_point(query_point)

            if not np.isnan(snapped).any():
                if abs(snapped[1] - center_y) < 0.5:
                    grid[i, j] = 1
...
def generate_pointnav_episode(
    ...
    while episode_count < num_episodes or num_episodes < 0:
        ...
        if is_compatible:
            ...
            local_grid = get_local_occupancy_grid(sim, source_position)
            grid_np = np.array(local_grid, dtype=np.uint8) * 255
            img = Image.fromarray(grid_np)
            save_dir = "grid_maps"
            os.makedirs(save_dir, exist_ok=True)
            img.save(os.path.join(save_dir, f"episode_{episode_count}.png"))
