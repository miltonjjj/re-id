...
"""sample target points in fov region"""
def sample_target_points( 
    ...
    # 获取 agent 在世界坐标系中的“前”向量（即 -Z 向量经过旋转）
    forward_vector = rot_matrix @ np.array([0, 0, -1])
    # 计算 XZ 平面上的偏航角
    # (此约定： 0 = -Z, pi/2 = +X)
    agent_yaw = math.atan2(forward_vector[0], -forward_vector[2])

    fov_rad = math.radians(fov)
    half_fov = fov_rad / 2.0
    start_angle = agent_yaw - half_fov
    end_angle = agent_yaw + half_fov
    angles = [
            start_angle + (end_angle - start_angle) * i / (num_angle_samples - 1)
    ...
    #for i in range(num_angle_samples):
        ...
        #     agent_pos[2] + dz
        # ]
        # angle = start_angle + (end_angle - start_angle) * i / (num_angle_samples - 1)
        # dx = sampling_distance * math.sin(angle)
        # dz = sampling_distance * math.cos(angle)
        dx = sampling_distance * math.sin(angle)
        dz = -sampling_distance * math.cos(angle)
...
def _create_multi_path_episode(
...
def generate_multi_path_episode(
    ...
    while episode_count < num_episodes or num_episodes < 0:
        ...
        target_points,original_navigable_target_count,snapped_navigable_target_count,not_navigable_target_count=sample_target_points(
            ...
            sampling_distance=0.05*sim.island_radius(source_position),
            fov=fov,
            sim=sim)#采样路径的尺寸大小
