...
def overlay_paths_on_rgb(rgb_image, all_paths, skeleton_shape, line_width=2):
    ...
    for i, path_info in enumerate(all_paths):
        ...
        cv2.circle(result_image, circle_center, radius=circle_radius, 
                  color=(0, 0, 255), thickness=1)  # 红色边框
...
def evaluate_paths_with_vlm_efficient(paths_rgb_path, path_metrics, examples_dir="examples"):
    """
    使用VLM高效评估paths_on_rgb图像质量
    
    Args:
        paths_rgb_path: 待评估的paths_on_rgb.png路径（已包含路径标记）
        path_metrics: 路径指标字典，格式为 {'path_1': {'path_number': 1, 'path_length_m': 2.5, 'tortuosity': 1.2}, ...}
        examples_dir: 示例图片目录
    ...
    try:
        ...
        # ========== Part 2: 加载Image Prompt ==========
        # 图像已经包含了路径可视化（白色路径 + 编号圆圈）
        with open(paths_rgb_path, 'rb') as f:
            img_bytes = f.read()
        img_b64 = base64.b64encode(img_bytes).decode('utf-8')

        # ========== Part 3: Score Prompt (加权细分打分设计) ==========
        path_data_lines = []
  ...
  "sub_scores": {{
    ...
    "exploration_score": <float 0-10>
        ...
        response = client.chat.completions.create(
            ...
            max_tokens=500, # 增加 token 以容纳详细分析
            temperature=0.4, # 略微增加温度，增加打分的随机性和批判性
        )
...
def process_single_episode(observations, episode_folder_path, scene_name, episode_num):
    ...
    try:
        ...
        # ========== VLM评估 ==========
        print("\n[VLM评估] 使用视觉语言模型评估路径质量...")
        
        vlm_result = evaluate_paths_with_vlm_efficient(paths_rgb_path, paths_metrics)

        if vlm_result:
            actual_tokens = vlm_result['token_usage']['total_tokens']
            print(f"    VLM评分: {vlm_result['score']:.1f}/10")
            print(f"    决策: {vlm_result['decision'].upper()}")
            print(f"    原因: {vlm_result['explanation']}")
            
            # 保存VLM评估结果
            vlm_result_path = os.path.join(episode_folder_path, 'vlm_evaluation.json')
            with open(vlm_result_path, 'w') as f:
                json.dump(vlm_result, f, indent=2)
            
            if vlm_result['decision'] == 'delete':
                print(f"    ⚠️  VLM建议删除此episode（得分: {vlm_result['score']}/10）")
                # 可选：直接返回False跳过后续处理
                # return False
        else:
            print(f"    ⚠️  VLM评估失败，继续处理...")
    except Exception as e:
        print(f"    ✗ 路径生成失败: {e}")
        return False
...
def main(config_path="benchmark/nav/pointnav/pointnav_hm3d.yaml", 
    ...
    print("="*70)
    print("完整Voronoi路径规划和动作生成Pipeline (with VLM)")
    print("="*70)
    ...
    processed_count = 0
    failed_count = 0
    vlm_keep_count = 0    # 总的VLM保留数量
    vlm_reject_count = 0  # 总的VLM删除数量
    ...
    for scene_idx, scene_name in enumerate(scene_names, 1):
        ...
        with habitat.Env(config=config, dataset=dataset) as env:
            ...
            scene_processed = 0
            scene_failed = 0
            scene_vlm_keep = 0    # 当前场景VLM保留数量
            scene_vlm_reject = 0  # 当前场景VLM删除数量
            ...
            for ep_idx in tqdm.tqdm(range(min(max_episodes_per_scene, total_episodes)), 
                ...
                if os.path.exists(episode_folder_path):
                    if all(os.path.exists(os.path.join(episode_folder_path, f)) 
                        ...
                        # 检查VLM评估结果
                        try:
                            vlm_path = os.path.join(episode_folder_path, 'vlm_evaluation.json')
                            with open(vlm_path, 'r') as f:
                                vlm_data = json.load(f)
                                decision = vlm_data.get('decision')
                                if decision == 'delete' or decision == 'no': # 兼容旧数据
                                    scene_vlm_reject += 1
                                elif decision == 'keep' or decision == 'yes':
                                    scene_vlm_keep += 1
                        except:
                            pass
                        continue
                    ...
                    if success:
                        ...
                        try:
                            ...
                            with open(vlm_path, 'r') as f:
                                vlm_data = json.load(f)
                                decision = vlm_data.get('decision')
                                if decision == 'delete' or decision == 'no':
                                    scene_vlm_reject += 1
                                elif decision == 'keep' or decision == 'yes':
                                    scene_vlm_keep += 1
                        except:
            ...
            print(f"  VLM保留 (Keep): {scene_vlm_keep}")
            print(f"  VLM删除 (Delete): {scene_vlm_reject}")
            
            processed_count += scene_processed
            failed_count += scene_failed
            vlm_keep_count += scene_vlm_keep
            vlm_reject_count += scene_vlm_reject
    ...
    print(f"  VLM总保留 (Keep): {vlm_keep_count}")
    print(f"  VLM总删除 (Delete): {vlm_reject_count}")
    print(f"  输出目录: {output_dir}")
    print("="*70)
